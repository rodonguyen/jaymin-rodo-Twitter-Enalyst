...
const server =
  "http://cab432-assignment-cloud.australiaeast.cloudapp.azure.com";
            ...
            trend.entityNames.forEach((keyword) => {
              ...
              redisClient.get(redisKey, (err, data) => {
                if (data) {
                  ...
                  documentDB
                    .queryCollection(
                      config.creds.trendCollection.id,
                      "id",
                      keyword
                    )
                    .then((results) => {
                      if (results.length > 0) {
                        console.log(
                          "The keyword exists in DocumentDB database, save to Redis"
                        );
                        redisClient.setex(
                          redisKey,
                          3600,
  ...
  return new Promise((resolve) => {
    redisClient.hgetall(redisKey, function (err, result) {
      if (result) {
        ...
        documentDB
          .queryCollection(config.creds.tweetCollection.id, column, tags)
          .then((results) => {
            if (results) {
              resolve(results);
            }
  ...
  return new Promise((resolve) => {
    redisClient.keys("cab432trends:*", function (err, trends) {
      if (trends) {
        ...
        documentDB
          .queryCollection(config.creds.trendCollection.id, "id", "")
          .then((results) => {
            if (results) {
  ...
  if (stream === null) {
    ...
    stream.on("tweet", function (tweet) {
      ...
      helper.parseTweets(tweet).then((result) => {
        helper
          .sentimentAnalysis(result, tags, tweet.id_str)
          .then((result) => {
            redisClient.hgetall(redisKey, (err, data) => {
              if (data) {
                if (data[result.id] === undefined) {
                  ...
                  documentDB.getDocument(
                    config.creds.tweetCollection.id,
                    result
                  );
                } else {
        ...
        await getColumn("all", tags)
          .then(async (data) => {
            if (data.length !== 0) {
              ...
              helper.saveCSV(
                helper.parseDataArray(good_sentiment),
                "public/javascripts/good_sentiment.csv"
              );
              helper.saveCSV(
                helper.parseDataArray(bad_sentiment),
                "public/javascripts/bad_sentiment.csv"
              );
            }
